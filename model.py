...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Aggregate information across all sequence positions (mean pooling)
        pooled = encoded.mean(dim=1)  # Shape: (batch_size, input_dim)

        # Outputs for classification and regression
        output_chemical = self.softmax(self.fc_chemical(pooled))  # Classification output
        output_concentration = self.fc_concentration(pooled)  # Regression output
